...
function findPrismaSchemaProblems(dataModel, options, usePlural) {
	return diffSchema(Object.fromEntries(Object.entries(getExpectedSchema(options, { usePlural })).map(([model, table]) => [clientProperty(model), table])), introspectPrismaDataModel(dataModel));
}
//#endregion
...
const prismaAdapter = (prisma, config) => {
		...
		const convertSelect = (select, model, join) => {
			...
			if (join) {
				...
				for (const [joinModel, joinAttr] of Object.entries(join)) {
					const key = getJoinKeyName(model, joinAttr.modelKey ?? joinModel, schema);
		...
		*/
		...
		return {
			...
			async findOne({ model, modelKey = model, where, select, join }) {
				...
				for (const [joinModel, joinAttr] of Object.entries(join ?? {})) {
					const key = getJoinKeyName(modelKey, joinAttr.modelKey ?? joinModel, schema);
			...
			async update({ model, modelKey = model, where, update }) {
				if (!db[model]) throw new BetterAuthError(`Model ${model} does not exist in the database. If you haven't generated the Prisma client, you need to run 'npx prisma generate'`);
				if (!hasRootUniqueWhereCondition(modelKey, where)) {
					const whereClause = convertWhereClause({
						model: modelKey,
						where,
						action: "updateMany"
	...
	return (options) => {
		...
		if (dataModel) registerSchemaCheck(instance, createSchemaCheck(() => findPrismaSchemaProblems(dataModel, options, config.usePlural), "prisma"), { runtimeEnabled: checksSchema(options) });
		return instance;
	};
};
//#endregion
