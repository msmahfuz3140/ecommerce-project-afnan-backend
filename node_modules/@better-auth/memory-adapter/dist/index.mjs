	...
	*/
	const buildAdapterFactory = (activeDb) => createAdapterFactory({
		...
		adapter: ({ getFieldName, getDefaultFieldName, options }) => {
			const applySortToRecords = (records, sortBy, modelKey) => {
			...
			function convertWhereClause(where, model, modelKey, join, select) {
				const baseRecords = (() => {
			...
			return {
				create: async ({ model, data }) => {
					if (options.advanced?.database?.generateId === "serial") data.id = activeDb[model].length + 1;
					if (!activeDb[model]) activeDb[model] = [];
					activeDb[model].push(data);
					return data;
				},
				findOne: async ({ model, modelKey = model, where, select, join }) => {
					const res = convertWhereClause(where, model, modelKey, join, select);
					if (join) {
						const resArray = res;
				...
				findMany: async ({ model, modelKey = model, where, sortBy, limit, select, offset, join }) => {
					const res = convertWhereClause(where || [], model, modelKey, join, select);
					if (join) {
						const resArray = res;
				...
				incrementOne: async ({ model, modelKey = model, where, increment, set }) => {
					const target = convertWhereClause(where, model, modelKey)[0];
					if (!target) return null;
