...
import { assertPasswordNotTooLong } from "../../utils/password.mjs";
...
const usernameImpl = (options, includeDisplayUsername) => {
	...
	return {
				...
				create: { async before(user, context) {
					...
					if (username) {
						...
						return { data: {
							...
							...includeDisplayUsername ? { displayUsername: displayUsername ? displayUsernameNormalizer(displayUsername) : username } : {}
						} };
					}
					return { data: {
						...user,
						...includeDisplayUsername && displayUsername ? { displayUsername: displayUsernameNormalizer(displayUsername) } : {}
					} };
				} },
		...
		}, includeDisplayUsername), options?.schema),
		hooks: { before: [
			{
				matcher(context) {
				...
				handler: createAuthMiddleware(async (ctx) => {
					...
					if (username !== void 0 && typeof username === "string") {
						...
						if (ctx.path === "/update-user" && options?.immutableUsername) {
							...
							const usernamesDiffer = session?.user.username !== normalizedUsername;
							if (hasUsername && usernamesDiffer) throw APIError.from("BAD_REQUEST", USERNAME_ERROR_CODES.USERNAME_IS_IMMUTABLE);
						}
					...
					if (!includeDisplayUsername) return;
